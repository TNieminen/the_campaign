...
- `notes/dnd/`
  ...
  - `greath-northwood/` — wilderness travel content for the Great Northwood: `mechanic.md` (the d20 travel roll), `encounters.md` (d20 1–10), the `loot/` folder (d20 11–20), `factions.md` (the realm's powers), and the `locations/` folder (faction sites found via loot).
    ...
    - `greath-northwood/locations/` — faction ruins and rare living Valvela camps (10 per faction) that can be discovered alongside a loot result. One file per site with YAML frontmatter (`name`, `faction`, `type`, `tier`, `status`, `summary`, `purpose`) and a themed description. The chance of a location and its `tier` (modest/notable/grand) scale with the loot value. See its `README.md`.
